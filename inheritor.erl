		...
		Module ->
			case(erlang:module_loaded(Module)) of
				true -> 
					...
					MergedExportsAST;
				_ -> erlang:exit(io:format("Unable to inherit from module '~p'. Reason: Unable to locate '~p' module. Is it loaded?~n", [Module, Module]))
			end
	end;
walk_ast([{attribute, _, inherit, Module}| RestAST], NewAST, Inherited, Functions) ->
	if
		Inherited == true -> erlang:exit(io:format("Unable to inherit from module '~p'. Reason: Already inheriting from a module.~n", [Module]));
		true ->
			put(import, Module),
